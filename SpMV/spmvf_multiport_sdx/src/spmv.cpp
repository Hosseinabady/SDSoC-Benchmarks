...
#pragma HLS DATAFLOW
	...
	u32 row_size = row_1_size + row_2_size + row_3_size + row_4_size + row_5_size + row_6_size + row_7_size + row_8_size;
	u32 i;
	for (i = 0; i < row_size; i++) {
	#pragma HLS PIPELINE
		if (i < row_1_size)
			y_fifo << y_1[i];
		else if (i < row_1_size + row_2_size)
			y_fifo << y_2[i - (row_1_size)];
		else if (i < row_1_size + row_2_size + row_3_size)
			y_fifo << y_3[i-(row_1_size + row_2_size)];
		else if (i < row_1_size + row_2_size + row_3_size + row_4_size)
			y_fifo << y_4[i-(row_1_size + row_2_size + row_3_size)];
		else if (i < row_1_size + row_2_size + row_3_size + row_4_size + row_5_size)
			y_fifo << y_5[i-(row_1_size + row_2_size + row_3_size + row_4_size)];
		else if (i < row_1_size + row_2_size + row_3_size + row_4_size + row_5_size + row_6_size)
			y_fifo << y_6[i-(row_1_size + row_2_size + row_3_size + row_4_size + row_5_size)];
		else if (i < row_1_size + row_2_size + row_3_size + row_4_size + row_5_size + row_6_size + row_7_size)
			y_fifo << y_7[i-(row_1_size + row_2_size + row_3_size + row_4_size + row_5_size + row_6_size)];
		else
			y_fifo << y_8[i-(row_1_size + row_2_size + row_3_size + row_4_size + row_5_size + row_6_size + row_7_size)];
	}

	for (i = 0; i < row_size; ) {
#pragma HLS PIPELINE
		float_ap_uint32_t f2int_1;
		float_ap_uint32_t f2int_2;
		float_ap_uint32_t f2int_3;
		float_ap_uint32_t f2int_4;

		uintbuswidth_t  y_128;

		if (i < row_size) {
			f2int_1.f = y_fifo.read();
		} else {
			f2int_1.f = 0;
		}
		...
		if (i+1 < row_size) {
			f2int_2.f = y_fifo.read();
		} else {
			f2int_2.f = 0;
		}
		y_128.range(63,  32)  = f2int_2.apint;


		if (i+2 < row_size) {
			f2int_3.f = y_fifo.read();
		} else {
			f2int_3.f = 0;
		}
		y_128.range(95,  64)  = f2int_3.apint;


		if (i+3 < row_size) {
			f2int_4.f = y_fifo.read();
		} else {
			f2int_4.f = 0;
		}
		y_128.range(127,  96) = f2int_4.apint;
		y[i/4] = y_128;
...
#pragma HLS PIPELINE
		DATA_TYPE x_tmp = x[i].range(31, 0);
		x_1_local[i] = x_tmp;
		x_2_local[i] = x_tmp;
		x_3_local[i] = x_tmp;
		x_4_local[i] = x_tmp;
