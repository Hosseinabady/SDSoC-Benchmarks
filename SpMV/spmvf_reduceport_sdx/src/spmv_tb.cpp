		   ...
		   if (line[0] != '%') {
			   ...
			   for (u32 i = 0; i < (row_size); i++) {
				   if(fgets(line,sizeof line,fp)!= NULL) {
					   ...
					   printf("SpMV_Ref: error rweading file 1\n");
				   }
			   }
...
void spmv_init( DATA_TYPE **values, u32 **indices, DATA_TYPE **x, u32 *row_size, u32 *col_size, u32 *data_size, char* inputFile) {
	...
	if (fp != NULL) {
	   ...
	   while(fgets(line,sizeof line,fp)!= NULL) {// read a line from a file
		   if (line[0] != '%') {
			   ...
			   *indices     = (u32*)sds_alloc_non_cacheable((*row_size+*data_size)*sizeof(u32));


			   unsigned int index;
			   for (u32 i = 0; i < (*row_size+*data_size); i++) {
					if(fgets(line,sizeof line,fp)!= NULL) {
						sscanf( line, "%d ", &index);
						(*indices)[i] = index;
					} else {
...
int main(int argc, char **argv) {
	...
	strcat(fpgaFileName, "/");
	strcat(fpgaFileName, "united_padded_II=");
...
//	char fpgaFileName[1000] = "united_padded_padded_fpga_csr_cusparse_";
//	strcat(fpgaFileName, argv[1]);
//	char goldenFileName[1000] = "fpga_csr_cusparse_";
//	strcat(goldenFileName, argv[1]);

	spmv_init( &values, &indices, &x, &row_size, &col_size, &data_size, fpgaFileName);
	...
	spmv_accel(values, indices, x, y_hw, row_size, col_size, data_size);
...
//		printf("data at  %d with value y_ hw = %f, should be y_ sw = %f \n",i,y_hw[i],y_sw[i]);
